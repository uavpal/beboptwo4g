...
main()
{
	...
	# background pinging of bebop2 via zt*, change LED to blue if ping ok - needed to override green LED after a successful mppd reconnect
...
switch_to_lte()
{
	...
	do
		...
		if ip route add 192.168.42.1/32 dev $zt_interface; then
			ulogger -s -t uavpal_sc2 "... added IP route for bebop2 via zerotier interface $zt_interface successfully"
			echo $zt_interface >/tmp/zt_interface
			break # break out of loop
		fi
		sleep 1
		if [ "$p" -eq "$zerotier_iface_timeout_seconds" ]; then
			ulogger -s -t uavpal_sc2 "... zerotier IP route for bebop2 could not be set - most probably due to zerotier daemon not bringing up the network interface zt* within $zerotier_iface_timeout_seconds seconds - switching back to Wi-Fi"
			switch_to_wifi
			return # back to main()
		fi
	done

	for p in `seq 1 $beboptwo4g_ping_retry`
	do
		ulogger -s -t uavpal_sc2 "... trying to ping bebop2 via 4G/LTE through zerotier (try $p of $beboptwo4g_ping_retry)"
		if ping -c 1 -I $zt_interface 192.168.42.1 >/dev/null 2>&1; then
			ulogger -s -t uavpal_sc2 "... successfully received ping echo from bebop2 via zerotier over 4G/LTE"
			break # break out of loop
		fi
		if [ "$p" -eq "$beboptwo4g_ping_retry" ]; then
			ulogger -s -t uavpal_sc2 "... could not ping bebop2 via zerotier over 4G/LTE in $beboptwo4g_ping_retry attempts - switching back to Wi-Fi"
			switch_to_wifi
			return # back to main()
		fi
	done
...
switch_to_wifi()
{
	...
	killall udhcpc
	ulogger -s -t uavpal_sc2 "*** idle on Bebop 2 Wifi (or at least trying to connect) ***"
}
